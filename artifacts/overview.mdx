...
- **Your library, for brand fonts.** Upload licensed or custom font files (`woff2`, `woff`, `ttf`, `otf`) to your [library](/library/media-in-your-work) and the agent can apply them by family name. Library fonts render for viewers automatically and are bundled into exports. When you ask for work carrying a brand's name, the agent checks your library for that brand's fonts first — a lookalike face is the typographic version of a redrawn logo. If the font isn't there yet, it uses a sensible substitute and tells you, so you can upload the real one.

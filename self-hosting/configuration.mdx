...
Most day-to-day configuration — model provider, API keys, storage — is done in the web app under **Settings → Providers**, not through environment variables (see [Providers](/self-hosting/providers)). Env vars matter mainly at deploy time: ports, security keys, and bootstrap values.
...
| `IMAGE_MODEL_PROVIDER` | Provider for image generation (OpenRouter) |
| `IMAGE_MODEL_ID` | Image model to use |

Set either `ANTHROPIC_API_KEY` alone, or the `MODEL_PROVIDER` / `MODEL_ID` / `MODEL_API_KEY` trio. Everything here can also be configured after boot in **Settings → Providers** — see [Providers](/self-hosting/providers).
...
| `STUDIOGRAPH_LIB_CDN_BASE` | Optional mirror for artifact render libraries (three.js/D3/Motion). The server normally fetches these once from jsDelivr at boot, verifies them against pinned hashes, and serves them itself; on an egress-restricted network, point this at an internal mirror of the same files — the hash check still applies |
